...
# Signal system
DL = jnp.array(2.01861755959241e23)
F_RANGE_V = (jnp.array(3.10436677), jnp.array(4397.00982335))
...
def ll_nomax_fn(x: Array) -> Array:
    ...
    """
    vb_h = unpack(x)
    f_range_h = get_f_range(vb_h, T_OBS)
...
t_start = time.time()
for _ in range(n_loops):
    ll_fn(sample_Mc_tTc()).block_until_ready()  # type: ignore
print((time.time() - t_start) / n_loops)
